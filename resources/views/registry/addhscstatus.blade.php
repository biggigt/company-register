...
@section('title','Добавить продукцию и статус')
@section('content')
    <div class="ui container">
        ...
        <div class="ui message">
            ...
            <p>Продукция и статус добавляется только к уже существующей записи в реестре. Таким образом, если предприятие уже было внесено в реестр, и оно занимается дополнительно каким-либо иным видом деятельности, только в этом случае следует добавлять продукцию и статус в данной форме.</p>
        </div>
        <div class="ui two column centered grid">
            <div class="column">
                <form class="ui form" action="{{route('registry.addhscstatus')}}" method = "post">
                    @csrf
                    <div class="field">
                        <label for="registry_record">Записи в реестре:</label>
                        <select name = "registry_record" id = "registry_record" class="ui search dropdown" required>
                            ...
                            @foreach($registry_records as $registry_record)
                                <option value="{{ $registry_record->id }}">{{$registry_record->registry->registryType}} -> {{ $registry_record->company[regnumber] }} -> {{ $registry_record->company[name] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field">
                        <label for="harmonizedsc">ТН ВЭД:</label>
                        <select name = "harmonizedsc" id = "harmonizedsc" class="ui search dropdown" required>
                    ...
                    <div class="field">
                        ...
                        <select name = "status" id = "status" class="ui search dropdown" required>
                            ...
                            @foreach($statuses as $status)
                                <option value="{{ $status->id }}"><i class="green circle icon"></i>{{ $status->status }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field">
                        <label for="country">Экспорт/импорт в/из страны:</label>
                    ...
                    <div class="field">
                        ...
                        <input type="text" placeholder="Введите название документа, дату и др" name = "act" id = "act" class="form-control" required>
                    </div>
                    <div class="field">
                        ...
                        <div class="ui calendar" name="cdate" id="cdate">
                            <div class="ui input left icon">
                                ...
                                <input type="text" name="c_date" id="c_date" placeholder="Дата">
                            </div>
                        </div>
                    </div>
                    <div class="field">
                        <label for="registry_type_description">Комментарий:</label>
                        <input type="text" placeholder="При необходимости добавьте комментарий" name = "registry_type_description" id = "registry_type_description" class="form-control">
                    </div>
